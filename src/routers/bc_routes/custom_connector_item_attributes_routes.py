...
def list_item_attributes(
    ...
    """Return item attribute records from BC (Pag51000). Unfiltered, unpaginated requests are cached for 5 minutes.

    Item is a large table — pass limit/offset (e.g. Airbyte's paginator) to fetch one bounded
    page at a time instead of the whole table in one response.
    """
    try:
